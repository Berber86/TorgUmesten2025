...
(async () => {
  ...
  // Все арты конкурентов существуют на диске.
  for (const g of groups) {
    assert.ok(g.art, "у группы «" + g.name + "» нет поля art");
    assert.ok(
      fs.existsSync(path.join(ROOT, "competitor_portraits", g.art)),
      "файл арта не найден: " + g.art,
    );
  }
  ...
  figures.forEach((fig, i) => {
    ...
    assert.ok(img.getAttribute("src").endsWith(group.art), "арт фигуры не совпадает с группой: " + comp.name);
