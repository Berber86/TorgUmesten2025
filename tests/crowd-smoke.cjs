...
(async () => {
  ...
  // Все арты конкурентов существуют на диске, и каждое имя группы
  // покрыто именным соответствием (хеш-фолбэк — только аварийный).
  for (const g of groups) {
    ...
    const covered = new Set(g.arts.flatMap((a) => a.for || []));
    const miss = g.members.filter((m) => !covered.has(m));
    assert.equal(miss.length, 0, "имена без своей фигуры в «" + g.name + "»: " + miss.join(", "));
  }
  ...
  for (const crowdEl of doc.querySelectorAll("#new-market-view .walk-crowd")) {
    const srcs = [...crowdEl.querySelectorAll("img")].map((i) => i.getAttribute("src"));
    assert.equal(new Set(srcs).size, srcs.length, "у прилавка одинаковые фигуры рядом");
  }
  ...
  const figSrcs = figures.map((f) => f.querySelector("img").getAttribute("src"));
  assert.equal(new Set(figSrcs).size, figSrcs.length, "в толпе торга одинаковые фигуры рядом");
