...
(() => {
  ...
  vaultCard.innerHTML = `<span class="vault-emblem">▣</span><div><span class="eyebrow">ВАША ИСТОРИЯ ПОД ЗАЩИТОЙ</span><h3>Находки приходят и уходят. Прогресс остаётся.</h3><p>Автосохранение, резервная копия и файл на случай переезда.</p></div><button id="studio-open-vault">Открыть сейф ${svg("arrow")}</button>`;
  ...
  resetGame = function () {
    ...
    if (
      !confirm(
        "Начать новую игру? Текущий прогресс станет резервной копией. Дневник и название лавки останутся.",
      )
    )
      return;
    try {
  ...
  reportButton.innerHTML = "Утренний выпуск " + svg("arrow");
